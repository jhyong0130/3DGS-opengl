...
namespace lbvh {
        ...
        __device__ inline HashType morton_code(const float3& point, const lbvh::AABB& extent, float resolution = 1024.0f) noexcept {
            float3 p = point;
            ...
            float dx = fmaxf(extent.max.x - extent.min.x, 1e-6f);
            float dy = fmaxf(extent.max.y - extent.min.y, 1e-6f);
            float dz = fmaxf(extent.max.z - extent.min.z, 1e-6f);
            ...
            if (!isfinite(p.x) || !isfinite(p.y) || !isfinite(p.z)) {
                printf("Invalid centroid : (%f, %f, %f)\n", p.x, p.y, p.z);
            }

            return morton_code(p, resolution);
        }

        __device__ inline HashType morton_code(const lbvh::AABB& box, const lbvh::AABB& extent, float resolution = 1024.0f) noexcept {
            auto p = centroid(box);
